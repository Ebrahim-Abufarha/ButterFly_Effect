...
function SidebarColors() {
  const colors = [
    ...
    { from: "zinc-800", to: "zinc-700", color: "gray" },
    { from: "blue-700", to: "cyan-500", color: "cyan" },
    { from: "emerald-500", to: "teal-400", color: "emerald" },
    { from: "orange-500", to: "yellow-500", color: "orange" },
    { from: "red-600", to: "orange-600", color: "red" }
  ];
  ...
  return (
    <>
      ...
      <div className="w-full text-center">
        ...
        <SocialShare 
          platform="twitter" 
          text="Check%20Argon%20Dashboard%20made%20by%20%40CreativeTim%20%23webdesign%20%23dashboard%20%23tailwindcss&amp;url=https%3A%2F%2Fwww.creative-tim.com%2Fproduct%2Fargon-dashboard-tailwind" 
          icon="fab fa-twitter"
        />
        <SocialShare 
          platform="facebook" 
          text="https://www.creative-tim.com/product/argon-dashboard-tailwind" 
...
function SocialShare({ platform, text, icon }) {
  const url = platform === 'twitter' 
    ? `https://twitter.com/intent/tweet?text=${text}`
    : `https://www.facebook.com/sharer/sharer.php?u=${text}`;

  return (
    <a href={url} className="inline-block px-5 py-2.5 mb-0 mr-2 font-bold text-center text-white align-middle transition-all border-0 rounded-lg cursor-pointer hover:shadow-xs hover:-translate-y-px active:opacity-85 leading-normal text-sm ease-in tracking-tight-rem shadow-md bg-150 bg-x-25 me-2 border-slate-700 bg-slate-700" target="_blank" rel="noreferrer">
      <i className={`mr-1 ${icon}`}></i> {platform === 'twitter' ? 'Tweet' : 'Share'}
    </a>
  );
}
